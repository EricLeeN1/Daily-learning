$(document).ready(function() {
	...
	window.faviconDataUrl = new Favico({
		dataUrl : function(url) {
			document.querySelector(".dataUrlTest").innerHTML = url;
			document.querySelector(".dataUrlTest").setAttribute("href", url);
		},
		elementId : 'badgeDataUrl'
	});
	...
	$('.favWebcam').bind('click', function(e) {
		...
		if (true) {
			...
			alert('摄像头功能只能支持 Chrome 和 Firefox :(');
			return;
		}
	...
	setTimeout(function() {
		...
		window.val_faviconDataUrl = 1;
		window.faviconDataUrl.badge(window.val_faviconDataUrl);
	}, 1500);
});
